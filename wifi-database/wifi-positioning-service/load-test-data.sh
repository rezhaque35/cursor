...
# Enhanced Test Scenarios Data (41-64)
echo "Loading Enhanced Test Scenarios Data..."

# Test Case 1: Very Strong Signal AP
aws dynamodb put-item \
    --table-name wifi_access_points \
    --endpoint-url http://localhost:8000 \
    --profile dynamodb-local \
    --item '{
        "mac_addr": {"S": "00:11:22:33:44:01"},
        "version": {"S": "20240411-130000"},
        "latitude": {"N": "37.775000"},
        "longitude": {"N": "-122.419500"},
        "altitude": {"N": "10.0"},
        "horizontal_accuracy": {"N": "5.0"},
        "vertical_accuracy": {"N": "2.0"},
        "confidence": {"N": "0.85"},
        "best_method": {"S": "proximity"},
        "methods_used": {"L": [{"S": "proximity"}]},
        "sample_count": {"N": "100"},
        "ssid": {"S": "StrongSignal_Test"},
        "frequency": {"N": "2437"},
        "signal_strength_avg": {"N": "-35.0"},
        "signal_strength_std": {"N": "1.0"},
        "geohash": {"S": "9q8yyk"},
        "status": {"S": "active"}
    }'

# Test Case 2: Strong Signal APs with Excellent Distribution
for i in {2..4}; do
    lat=$(echo "37.775000 + ($i-2)*0.000100" | bc)
    lon=$(echo "-122.419500 + ($i-2)*0.000100" | bc)
    aws dynamodb put-item \
        --table-name wifi_access_points \
        --endpoint-url http://localhost:8000 \
        --profile dynamodb-local \
        --item '{
            "mac_addr": {"S": "00:11:22:33:44:0'$i'"},
            "version": {"S": "20240411-130100"},
            "latitude": {"N": "'$lat'"},
            "longitude": {"N": "'$lon'"},
            "altitude": {"N": "12.0"},
            "horizontal_accuracy": {"N": "8.0"},
            ...
            "methods_used": {"L": [{"S": "trilateration"}, {"S": "weighted_centroid"}]},
            "sample_count": {"N": "80"},
            "ssid": {"S": "Strong_AP_'$((i-1))'"},
            "frequency": {"N": "'$((5180 + (i-2)*20))'"},
            "signal_strength_avg": {"N": "'$((i-2-52))'"},
            "signal_strength_std": {"N": "1.5"},
            "geohash": {"S": "9q8yyk"},
            "status": {"S": "active"}
        }'
...
# Test Case 3: Medium Signal APs with Good Distribution
for i in {5..8}; do
    lat=$(echo "37.775200 + ($i-5)*0.000080" | bc)
    lon=$(echo "-122.419700 + ($i-5)*0.000080" | bc)
    aws dynamodb put-item \
        --table-name wifi_access_points \
        --endpoint-url http://localhost:8000 \
        --profile dynamodb-local \
        --item '{
            ...
            "methods_used": {"L": [{"S": "maximum_likelihood"}, {"S": "weighted_centroid"}]},
            "sample_count": {"N": "60"},
            "ssid": {"S": "Medium_AP_'$((i-4))'"},
            "frequency": {"N": "'$((2412 + (i-5)*25))'"},
            "signal_strength_avg": {"N": "'$((i-5-68))'"},
            "signal_strength_std": {"N": "2.0"},
            "geohash": {"S": "9q8yyk"},
            "status": {"S": "active"}
        }'
...
# Test Case 4: Weak Signal APs
for i in {9..11}; do
    lat=$(echo "37.775400 + ($i-9)*0.000060" | bc)
    lon=$(echo "-122.419900 + ($i-9)*0.000060" | bc)
    aws dynamodb put-item \
        --table-name wifi_access_points \
        --endpoint-url http://localhost:8000 \
        --profile dynamodb-local \
        --item '{
            ...
            "sample_count": {"N": "40"},
            "ssid": {"S": "Weak_AP_'$((i-8))'"},
            "frequency": {"N": "'$((5500 + (i-9)*20))'"},
            "signal_strength_avg": {"N": "'$((i-9-78))'"},
            "signal_strength_std": {"N": "3.0"},
            "geohash": {"S": "9q8yyk"},
            "status": {"S": "active"}
        }'
done

# Test Case 5: Very Weak Signal APs
for i in {12..13}; do
    lat=$(echo "37.775600 + ($i-12)*0.000040" | bc)
    lon=$(echo "-122.420100 + ($i-12)*0.000040" | bc)
    aws dynamodb put-item \
        --table-name wifi_access_points \
        --endpoint-url http://localhost:8000 \
        --profile dynamodb-local \
        --item '{
            "mac_addr": {"S": "00:11:22:33:44:'$i'"},
            "version": {"S": "20240411-130400"},
            "latitude": {"N": "'$lat'"},
            "longitude": {"N": "'$lon'"},
            "altitude": {"N": "20.0"},
            "horizontal_accuracy": {"N": "40.0"},
            "vertical_accuracy": {"N": "12.0"},
            "confidence": {"N": "0.35"},
            "best_method": {"S": "rssi_ratio"},
            "methods_used": {"L": [{"S": "rssi_ratio"}, {"S": "weighted_centroid"}]},
            "sample_count": {"N": "20"},
            "ssid": {"S": "VeryWeak_AP_'$((i-11))'"},
            "frequency": {"N": "'$((2412 + (i-12)*25))'"},
            "signal_strength_avg": {"N": "'$((i-12-88))'"},
            "signal_strength_std": {"N": "4.0"},
            "geohash": {"S": "9q8yyk"},
            "status": {"S": "active"}
        }'
done

# Test Case 6: Mixed Band Enterprise Network APs
for i in {14..17}; do
    lat=$(echo "37.775800 + ($i-14)*0.000050" | bc)
    lon=$(echo "-122.420300 + ($i-14)*0.000050" | bc)
    freq=$((i % 2 == 0 ? 2412 + (i-14)*25 : 5180 + (i-14)*40))
    band=$([ $((i % 2)) -eq 0 ] && echo "2.4G" || echo "5G")
    aws dynamodb put-item \
        --table-name wifi_access_points \
        --endpoint-url http://localhost:8000 \
        --profile dynamodb-local \
        --item '{
            ...
            "confidence": {"N": "0.70"},
            "best_method": {"S": "maximum_likelihood"},
            "methods_used": {"L": [{"S": "maximum_likelihood"}, {"S": "trilateration"}]},
            "sample_count": {"N": "90"},
            "ssid": {"S": "Enterprise_'$band'_'$((i-13))'"},
            "frequency": {"N": "'$freq'"},
            "signal_strength_avg": {"N": "'$((i-14-65))'"},
            "signal_strength_std": {"N": "2.0"},
            "geohash": {"S": "9q8yyk"},
            "status": {"S": "active"}
        }'
...
echo "Enhanced test data loading completed." 